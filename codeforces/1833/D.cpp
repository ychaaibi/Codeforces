...
void    done(vector<int>& ans, int l, int r)
{
    vector<int> nw(ans.size());

    int _l = l, _r = r;

    while (_l < _r and _l>=0 and _r < ans.size())
    {
        swap(ans[_l], ans[_r]);
        _l++, _r--;
    }
    ...
    while (i < ans.size() and j < ans.size())
        nw[i++] = ans[j++];
    j = l;
    while (i < ans.size() and j <= r)
        nw[i++] = ans[j++];
    j = 0;
    while (j < l)
        nw[i++] = ans[j++];
    for (int i=0; i<ans.size(); i++) ans[i] = nw[i];
}
...
void    solve()
{
    ...
    for (int i=0; i<r + (r == n - 1); i++)
    {
        ...
        // cout << endl;
        my_anx = max(my_anx, test);
    }
    vector<int> test = ans;
    for (int i=0; i<n; i++) cout << my_anx[i] << " ";
    cout << endl;
    // done(ans,l + 1,r - 1);
}
