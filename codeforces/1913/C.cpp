...
bool    check_val( ll val, vector<int> v)
{
    if ( !val ) return ( true );
    ...
    while ( val )
    {
        ...
        while ( val % 2 == 0 and val ) val /= 2, pw++;
        // cout << pw << " " << prev << endl;
        seq.push_back( pw + prev );
    ...
    // cout << endl;
    int p1_pow = 0, p2_seq = 0;

    while ( p2_seq < seq.size() )
    {
        if ( p1_pow < seq[ p2_seq ] )
        {
            p1_pow++;
            v[ p1_pow ] += ( v[ p1_pow - 1 ] / 2 );
        }
        else if ( v[ p1_pow ] )
        {
            v[ p1_pow ]--;
