...
void    solve()
{
    ...
    for (int i=0; i<s.length(); i++)
    {
        ...
        if (!ans)
        {
            for (; st <= i; st++) c_ans[st] = ((s[i] == ')') ? 1 : 2);
            st = i + 1;
        }
        else if (ans < 0) neg = true;
        else pos = true;
    }
    if (ans) cout << -1 << endl;
    else cout << neg + pos << endl;
    for (int i=0; i<c_ans.size() and !ans; i++) cout << ((neg and pos) ? c_ans[i] : 1) << " ";
    if (!ans) cout << endl;
