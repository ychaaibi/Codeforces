...
void    solve()
{
    ...
    for (int i=0; i<m; i++) cin >> v_m[i];
    srt(v), srt(v_m);
    
    long long   ans = 0;
    int p1_v = 0, p2_v = n - 1, p1_v_m = 0, p2_v_m = m - 1;

    while ( p1_v <= p2_v )
    {
        if ( abs(v[p1_v] - v_m[p2_v_m]) > abs( v[p2_v] - v_m[p1_v_m] ) )
        {
            ans += abs(v[p1_v] - v_m[p2_v_m]);
            p1_v++, p2_v_m--;
        }
        else
        {
            ans += abs( v[p2_v] - v_m[p1_v_m] );
            p2_v--, p1_v_m++;
        }
    }
    cout << ans << endl;
