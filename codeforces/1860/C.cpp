...
void    solve()
{
    ...
    int i = 1;
    int least_no_lucky = v[0];
    for (; i<n and v[i] < v[i - 1];)
    {
        least_no_lucky = min(least_no_lucky, v[i]);
        i++;
    }
    int ans = 0;
    int least_lucky = INT_MAX;
    for (; i<n; i++)
    {
        if ((least_lucky == INT_MAX or least_lucky > v[i]) and v[i] > least_no_lucky)
            least_lucky = min(least_lucky, v[i]), ans++;
        else
            least_no_lucky = min(least_no_lucky, v[i]);
    }
    cout << ans << endl;
