...
void    solve()
{
    ...
    for (int i=n; i>=1; i--)
    {
        if (((i % 2) and v[i] >= 0) or (!(i % 2) and v[i] <= 0))
        {
            st.resize(0);
            if (i % 2)
                ans += v[i];
    ...
    if (st.size() < 2 or sum == 0)
    {
        cout << ans << endl;
        return ;
    }
    if (st[0] >= 0) ans += sum - st[0];
    else
    {
        int mn = INT_MAX;
        int val = -1;
        for (int i=0; i<st.size() and st[i] < 0; i++) mn = min(abs(st[i]), mn);
        ...
        if (val != -1) ans = max(max(ans + sum - mn, ans + sum - val), ans);
        else ans = max(ans, ans + sum - mn);
    }
    cout << ans << endl;
}
