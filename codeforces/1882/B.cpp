...
int     get_ans(vector<vector<int>>& S, vector<int>& exist)
{ 
    ...
    {
        if (binary_search(exist.begin(), exist.end(), i)) continue ;
...
void    solve()
{
    ...
    {
        if (exist[i].empty()) continue ;
        ans = max(ans, get_ans(S, exist[i]));
    }
    cout << ans << endl;
}
