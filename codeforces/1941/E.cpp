...
void    solve()
{
    ...
    cin >> n >> m >> k >> d;
    vector<vector<long long>> rows(n, vector<long long>(m));
    ...
    vector<long long> dp_rows(n);
    vector<long long> dp(m);
    
    for ( int i=0; i<n; i++ ) dp_rows[i] = construct_row( rows[i], dp, d ) + (( i >= 1) ? dp_rows[i - 1] : 0);

// cout << endl;
    // for ( int i=0; i<n; i++ ) cout << dp_rows[i] << " ";
    // cout << endl;

    long long   mn = dp_rows[k - 1];
    // cout << mn << endl;

    for ( int i=k; i<n; i++)
    {
        // cout << dp_rows[i] << " " << dp_rows[i - k] << endl;
        mn = min(mn, dp_rows[i] - dp_rows[i - k]);
        // cout << mn << endl;
    }
    cout << mn << endl;
}
