...
void    find(ll n, ll mlt, vector<pair<ll, ll> > div, int i)
{
    ans = min(ans, max(mlt, (ll)n / mlt));

    if (i == div.size()) return ;

    find(n, (ll)mlt * pow(div[i].f, div[i].s), div, i + 1);
    find(n, mlt, div, i + 1);
}
