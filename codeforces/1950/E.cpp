...
string  get_syc( string s, int div )
{
    unordered_map<string,int> mp;

    for ( int k=0; k<s.length() / div; k++ )
    {
        mp[s.substr(div * k, div)]++;
    }

    string ans = s.substr(0,div);
...
void    solve()
{
    ...
    vector<int> div = find_all_divisors(n);

    srt(div);

    int st=0, en=div.size();
    int ans = div.size() - 1;
...
int main()
{
    ...
    // {
    //     mx = max(mx, find_all_divisors(i).size());
    // }
    // // cout << mx << endl;
    return (0);
}
