...
/*   Updated: 2022/12/05 12:22:13 by ychaaibi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    solve()
{
    ...
    for (int i=1; i<101; i++)
    {
        ...
        int count = min(v_b[i], v_g[i - 1]);
        ans += count, v_b[i] -= count, v_g[i - 1] -= count;
        count = min(v_b[i], v_g[i]);
        ans += count, v_b[i] -= count, v_g[i] -= count;
        count = min(v_b[i], v_g[i + 1]);
        ans += count, v_b[i] -= count, v_g[i + 1] -= count;
    }
    cout << ans << endl;
}
