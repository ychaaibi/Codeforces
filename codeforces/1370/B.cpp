...
/*   Updated: 2022/11/30 14:22:00 by ychaaibi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    solve()
{
    ...
    int index_impair = -1;
    int count = n - 1;
    for (int i=0; i<2 * n and count; i++)
    {
        if (v[i] % 2 and index_impair != -1)
            cout << index_impair + 1 << " " << i + 1 << endl, count--, index_impair = -1;
        else if (v[i] % 2 == 0 and index_pair != -1)
            cout << index_pair + 1 << " " << i + 1 << endl, count--, index_pair = -1;
        else if (v[i] % 2)
            index_impair = i;
        else
            index_pair = i;
