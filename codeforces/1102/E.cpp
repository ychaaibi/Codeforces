...
void    solve()
{
    ...
    for (int i=0; i<v.size(); i++)
    {
        ...
        {
            if (i - 1 - start >= 1)
                v_comb.push_back(mp(v[start].s, v[i - 1].s));
            start = i;
        }
    }
    if (start != v.size() - 1)
        v_comb.push_back(mp(v[start].s, v[v.size() - 1].s));
    int count = 0;
    ull ans = 1;
    int j = 0;
    v_comb = merge_intervals(v_comb);
    if (!v_comb.size())
        count = n - 1;
    else
    {
        if (v_comb[0].f != 0)
            count = v_comb[0].f;
        for (int i=1; i<v_comb.size(); i++)
            count += v_comb[i].f - v_comb[i - 1].s;
        count += v.size() - 1 - v_comb[v_comb.size() - 1].s;
    }
    for (int i=0; i<count; i++) ans = (ans << 1) % M;
    cout << ans << endl;
}
