...
void    solve()
{
    ...
    for ( int i=2; i<n; i++ )
    {
        if ( v[ i ] - v[ i - 1 ]  > v[ i - 1 ] - v[ i - 2 ] )
            pref_sc[ i ] = pref_sc[ i - 1 ] + ( v[ i ] - v[ i - 1 ] );
        else
            pref_sc[ i ] = pref_sc[ i - 1 ] + 1;
    }
    ...
    for ( int i=n-3; i>=0; i-- )
    {
        if ( v[ i + 1 ] - v[ i ] > v[ i + 2 ] - v[ i + 1 ] )
            suff_sc[ i ] = suff_sc[ i + 1 ] + ( v[ i + 1 ] - v[ i ] );
        else
            suff_sc[ i ] = suff_sc[ i + 1 ] + 1;
    }
    int q;
    ...
    while ( q-- )
    {
        ...
        if ( y < x )
            cout << suff_sc[ y - 1 ] - suff_sc[ x  - 1 ] << endl;
        else
            cout << pref_sc[ y - 1 ] - pref_sc[ x - 1 ] << endl;
    }
}
