...
/*   Updated: 2022/12/07 12:59:59 by ychaaibi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void display(deque<int> const &deque, string s)
{
...
void    solve()
{
    ...
    for (int i=0; i<s.length(); i++)
    {
        if (n)
        {
            while (st.size() > 1 and n and s[st.back()] > s[i])
            {
                n--;
                st.pop_back();
            }
            if (st.size() and s[i] != '0' and s[st.back()] > s[i] and n)
            {
                n--;
                st.pop_back();
            }
            if (st.size() and s[i] != '0' and s[st.back()] == '0' and s[i] < s[st.front()] and st.size() <= n)
            {
                deque<int>  st_new;
    ...
    while (n-- and st.size())
        st.pop_back();
    for (int i=0; i<st.size(); i++) cout << s[st[i]];
    cout << endl;
}
