...
ll  ans_check(vector<ll>& check, int k)
{
    ll mn = check[k - 1], mx = check[k - 1];

    for (int i=2 * k - 1; i<check.size(); i += k)
    {
        mn = min(mn, check[i] - check[i - k]);
        mx = max(mx, check[i] - check[i - k]);
    }
    return (mx - mn);
}
...
void    solve()
{
    ...
    for (int i=1; i<=n / 2; i++)
    {
        if (n % i == 0) ans = max(ans, ans_check(v, i));
    }
    cout << ans << endl;
}
