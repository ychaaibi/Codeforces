...
/*   Updated: 2022/11/30 10:44:05 by ychaaibi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    solve()
{
    ...
    cin >> s;
    int     ab_f = -1, ba_f = -1, ab_l = -1, ba_l = -1;

    for (int i=1; i<ln(s); i++)
    {
        if (ab_f != -1 and ba_f != -1) break ;
        if (s[i - 1] == 'A' and s[i] == 'B' and ab_f == -1)
            ab_f = i - 1;
        else if (s[i - 1] == 'B' and s[i] == 'A' and ba_f == -1)
            ba_f = i - 1;
    }

    for (int i=ln(s) - 1; i>0; i--)
    {
        if (ab_l != -1 and ba_l != -1) break ;
        if (s[i - 1] == 'A' and s[i] == 'B' and ab_l == -1)
            ab_l = i - 1;
        else if (s[i - 1] == 'B' and s[i] == 'A' and ba_l == -1)
            ba_l = i - 1;
    }
    if (ba_f == -1 or ab_f == -1)
    {
    ...
    if (abs(ba_f - ab_l) > 1 or abs(ab_f - ba_l) > 1)
        cout << "YES" << endl;
    else
        cout << "NO" << endl;
}
